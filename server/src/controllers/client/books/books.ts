...
class Books {
   ...
   */
  static async createBooks(req: any, res: any, next: any) {
    ...
    try {
      ...
      if (Number(reqData.is_free) !== isFree.free) {
        ...
        if (reqData.price > 100) {
          throw new Error('小书当前定价不能超过100，后续等待管理员开放！')
        }
   ...
   * @param   {object} ctx 上下文对象
   */
  static async getUserBooksInfo(req: any, res: any, next: any) {
   ...
   * @param   {object} ctx 上下文对象
   */
  static async updateBooks(req: any, res: any, next: any) {
    let reqData = req.body
    let { user = '' } = req
    try {
      if (!reqData.title) {
  ...
  static async deleteBooks(req: any, res: any, next: any) {
  ...
  static async getBooksList(req: any, res: any, next: any) {
   ...
   * @param   {object} ctx 上下文对象
   */
  static async getBooksInfo(req: any, res: any, next: any) {
   ...
   * @param   {object} ctx 上下文对象
   */
  static async getBooksBookAll(req: any, res: any, next: any) {
    let { books_id } = req.query
    let { user = '', islogin } = req
  ...
  static async getCollectBooksList(req: any, res: any, next: any) {
    let page = req.query.page || 1
    let pageSize = req.query.pageSize || 24
