...
const lowdbRead: any = lowdb.read()
...
const sendMail = function (recipient: string, subject: string, html: string) {
  smtpTransport.sendMail(
    ...
    function (error: any, response: string) {
      if (error) {
        console.log(error)
      }
      console.log('发送成功')
...
const sendVerifyCodeMail = function (recipient: string, subject: string, code: string) {
...
const sendNotification = (recipient: string, subject: string, data: any) => {
  // 消息通知
  return new Promise((resolve, reject) => {
    smtpTransport.sendMail(
      {
