const { sequelize, article_category, article } = require('../models')
...
class Users {
   ...
   */
  static async get_article_category_list(ctx) {
    const { page = 1, pageSize = 10 } = ctx.query
    let { count, rows } = await article_category.findAndCountAll({
   ...
   * @param   {obejct} ctx 上下文对象
   */
  static async edit_article_category(ctx) {
    const { uid, nickname, password, account, enable } = ctx.request.body
    await article_category.update({
 ...
 * 管理员对角色是一多一的关系
 */
  static async delete_article_category(ctx) {
    ...
    if (!find_article) { /* 无关联 */
      await article_category.destroy({ 'where': { uid } })
        .then(function (p) {
          format_data(ctx, {
            state: 'success',
            message: '删除用户成功'
      ...
      await sequelize.transaction(function (transaction) {
        // 在事务中执行操作
        return article_category.destroy({ where: { uid } }, { transaction })
