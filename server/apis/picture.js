...
class Picture {
  ...
  static async create_picture (ctx) {
    ...
    try {
      let find_picture_title = await picture.findOne({where: {picture_title: req_data.picture_title}})
  ...
  static async get_picture_list (ctx) {
    ...
    let {count, rows} = await picture.findAndCountAll({
      attributes: ['picture_id', 'picture_title', 'picture_url', 'picture_type', 'enable'],
  ...
  static async update_picture (ctx) {
    ...
    await picture.update({
      picture_title: req_data.picture_title,
      picture_url: req_data.picture_url[0].response ? req_data.picture_url[0].response.data.filename : req_data.picture_url,
      picture_type: req_data.picture_type,
      enable: req_data.enable
    }, {
      where: {
