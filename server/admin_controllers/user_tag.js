...
class User_Tag {
  ...
  static async create_user_tag (ctx) {
    ...
    try {
      let find_user_tag_name = await user_tag.findOne({where: {user_tag_name: req_data.user_tag_name}})
  ...
  static async get_user_tag_all (ctx) {
    let user_tag_all = await user_tag.findAll({
      attributes: ['user_tag_id', 'user_tag_name', 'user_tag_description', 'user_tag_icon', 'user_tag_icon_type'],
