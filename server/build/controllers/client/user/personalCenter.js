    ...
    return __awaiter(this, void 0, void 0, function*() {
      ...
      try {
        ...
        for (let i in rows) {
          rows[i].setDataValue(
            'create_dt',
            yield TimeDistance(rows[i].create_date)
          )
          ...
          if (rows[i].tag_ids) {
            rows[i].setDataValue(
              'tag',
              yield models.article_tag.findAll({
                where: {
    ...
    return __awaiter(this, void 0, void 0, function*() {
      ...
      try {
        ...
        for (let i in rows) {
          ...
          rows[i].setDataValue(
            'userAttentionIds',
            yield models.attention.findAll({
              where: {
    ...
    return __awaiter(this, void 0, void 0, function*() {
      ...
      try {
        ...
        for (let i in rows) {
          ...
          rows[i].setDataValue(
            'update_dt',
            yield TimeDistance(rows[i].update_dt)
          )
          rows[i].setDataValue(
            'articleCount',
            yield models.article.count({
              where: { blog_ids: rows[i].blog_id }
            })
          )
          rows[i].setDataValue(
            'likeCount',
            yield models.collect.count({
              where: {
