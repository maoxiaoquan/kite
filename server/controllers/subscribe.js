...
class Subscribe {
  ...
  static async render_subscribe_tag (ctx) {
    ...
    let find_subscribe_article_tag = ctx.session.uid ?
      await models.subscribe_article_tag.findAll({where: {uid: ctx.session.uid}}).then((res) => {
    ...
    if (Number(is_subscribe) === 1) { /*all 的标签*/
      find_where = tag_name ? {enable: 1, article_tag_name: {[Op.like]: `%${tag_name}%`}} : {enable: 1}
    } else { /*自己的 的标签*/
      find_where = tag_name ?
        {
          enable: 1,
    ...
    let {count, rows} = await models.article_tag.findAndCountAll({
      ...
      res.rows.map(async (item, key) => {
        item.subscribe_count = await models.subscribe_article_tag.count({where: {article_tag_id: item.article_tag_id}})
  ...
  static async post_subscribe_tag (ctx) {
    ...
    if (findone_subscribe_article_tag) { /*判断是否关注了，是则取消，否则添加*/
