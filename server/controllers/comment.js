...
class Comment {
  ...
  static async post_create_comment (ctx) {
    ...
    try {
      await models.comment.create({
        ...
        await models.article.update({ // 更新文章评论数
          comment_count: await models.comment.count({where: {aid: formData.aid, parent_id: 0}})
        }, {'where': {aid: formData.aid}})

        const user_info = await models.user.findOne({where: {uid: data.uid}}) // 查询当前评论用户的信息
  ...
  static async post_delete_comment (ctx) {
    ...
    try {
      ...
      if (find_children_comment_id.length > 0) { // 判断当前评论下是否有子评论,有则删除子评论
