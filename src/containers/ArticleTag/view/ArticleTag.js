...
class ArticleTag extends React.Component {
  ...
  fetch_create_article_tag = (values) => {  /*创建标签*/
    this.props.dispatch(create_article_tag(values, (res) => {
  ...
  fetch_update_article_tag = (values) => { /*修改标签*/
  ...
  fetch_article_tag_list = () => {  /*获取管理员用户带分页的列表*/
    const that = this
    this.setState({loading: true})
    const {pagination: {current}} = this.state
    this.props.dispatch(get_article_tag_list({params: {page: current}}, (res) => {
          ...
          >
            ...
            >
              ...
              <FormItem
                ...
                label="标签图标类型"
              >
                {getFieldDecorator('article_tag_icon_type', {
                  rules: [
